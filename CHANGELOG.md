...
transport defects from the 1.0.0 audit; raises the minimum supported Android
version; adds new typed rejection and exception paths so operators see
encryption failures instead of silent plaintext fallbacks.

### Changed (breaking)

- `minSdk` raised from 31 to 33 (Android 13). Devices on Android 12 / 12L are
  no longer supported.
- `DatabaseEncryptionProvider.createFactory()` now takes a `Context` and the
  SQLCipher passphrase is a 256-bit random value wrapped with the Android
  Keystore AES-256-GCM key (was: `key.encoded` with a broken `.toString()`
  fallback that produced a non-random passphrase on hardware-backed devices).
  Databases encrypted under 1.0.0 cannot be opened on 1.1.0; operators must
  wipe and recreate the Room databases on first launch after upgrade.
- `FieldLevelEncryptor.encryptFields` / `decryptFields` throw
  `FieldEncryptionException` on failure instead of returning the original
  plaintext payload. Events that require field-level encryption but fail the
  operation are now rejected with the new
  `EnqueueResult.Rejected.FieldEncryptionFailed`.
- `MtlsClientBuilder.build` throws `MtlsConfigurationException` on failure
  instead of silently downgrading to an unauthenticated TLS client.
- `CertificateTransparencyValidator` no longer treats a known-CA issuer DN
  as a substitute for embedded SCT presence; the issuer-string bypass was
  not a security boundary and has been removed.

### Security

- `DataRightsManager.wipeAllSdkData` now enumerates the app's `shared_prefs`
  directory and removes every `kioskops*` preferences file plus every Android
  Keystore alias under `kioskops*`. Prior behavior left key metadata, device
  identifiers, and attestation signing keys in place after an Art. 17 wipe.
- Key attestation challenge generated by `KeystoreAttestationProvider` is now
  32 cryptographically random bytes (was: `Build.MODEL:Build.MANUFACTURER:currentTimeMillis()`
  padded to 32 bytes, which allowed trivial replay of attestation blobs
  across same-model devices).
- `KioskOpsConfig.toString()` redacts `adminExitPin`. Previous default data-class
  `toString` included the PIN, allowing it to leak via logs, crash reports, or
  diagnostics bundles.
- `VersionedCryptoProvider.deleteKeyVersion` now logs Keystore deletion
  failures and retains metadata so a future cleanup cycle can retry. Silent
  deletion failures previously orphaned Keystore entries.

### Removed

- `CertificateTransparencyValidator.isFromKnownCa` (internal) and its
  hard-coded CA issuer list.

### Build

- Gradle wrapper validation workflow retained; binary artifact (gradle-wrapper.jar)
  scorecard alert marked as expected-exception.
- SDK module still publishes 1.0.0-API-compatible public surface except for
  the listed breaking changes.
